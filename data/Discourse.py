...
class Discourse:
    def __init__(self, sentences, sentences_tags, sent_types, total_words, total_tags):
    ...
    def evaluate(self, other, S, N, R, F):
        S.overall_label_count += len(self.result.subtrees)
        S.predicated_label_count += len(self.result.subtrees)
        for p_subtree in other.subtrees:
            for g_subtree in self.result.subtrees:
                if p_subtree.spanEqual(g_subtree):
                    S.correct_label_count += 1
                    break

        N.overall_label_count += len(self.result.subtrees)
        N.predicated_label_count += len(self.result.subtrees)
        for p_subtree in other.subtrees:
            for g_subtree in self.result.subtrees:
                if p_subtree.nuclearEqual(g_subtree):
                    N.correct_label_count += 1
                    break

        R.overall_label_count += len(self.result.subtrees)
        R.predicated_label_count += len(self.result.subtrees)
        for p_subtree in other.subtrees:
            for g_subtree in self.result.subtrees:
                if p_subtree.relationEqual(g_subtree):
                    R.correct_label_count += 1
                    break

        F.overall_label_count += len(self.result.subtrees)
        F.predicated_label_count += len(self.result.subtrees)
        for p_subtree in other.subtrees:
            for g_subtree in self.result.subtrees:
                if p_subtree.fullEqual(g_subtree):
