...
class DisParser(object):
    ...
    def encode(self, edu_words, edu_extwords, edu_tags, word_mask, edu_mask, word_denominator, edu_types):
        if self.use_cuda:
            edu_words = edu_words.cuda()
            edu_extwords = edu_extwords.cuda()
            edu_tags = edu_tags.cuda()
            word_mask = word_mask.cuda()
            edu_mask = edu_mask.cuda()
            word_denominator = word_denominator.cuda()
            edu_types = edu_types.cuda()
        word_represents = self.wordEnc(edu_words, edu_extwords, edu_tags, word_mask)
        self.edu_represents = self.EDUEnc(word_represents, edu_mask, word_denominator, edu_types)
    ...
    def hidden_prepare(self, batch_feats):
        ...
        bucket = Variable(torch.zeros(batch_size, 1, hidden_size)).type(torch.FloatTensor)
        if self.use_cuda:
            bucket = bucket.cuda()
        edu_rep = torch.cat((self.edu_represents, bucket), 1) # batch_size, action_num + 1, hidden_size

        stack_index = Variable(torch.ones(batch_size * action_num * 3 * EDU_num)).type(torch.LongTensor) * EDU_num
        stack_denominator = Variable(torch.ones(batch_size * action_num * 3)).type(torch.FloatTensor) * -1
        queue_index = Variable(torch.ones(batch_size * action_num)).type(torch.LongTensor) * EDU_num
